...
class TopicModel:
    ...
    def sample(self, iterations):
        cdef
        int
        old_topic, new_topic, word_id, topic, i, doc_length
        cdef
        double
        sampling_sum = 0
        cdef
...
doc_topics, long[:]
topic_changes, long[:]
doc_topic_counts, long[:, :]
word_topics, long[:]
topic_totals, double[:]
topic_probs, double[:]
topic_normalizers, float
doc_smoothing, float
word_smoothing, float
smoothing_times_vocab_size, int
num_topics):

cdef
int
old_topic, new_topic, word_id, topic, i
cdef
double
sampling_sum = 0
cdef
