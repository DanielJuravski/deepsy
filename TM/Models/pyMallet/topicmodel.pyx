...
cdef class TopicModel:
            ...
            for document in self.documents:
                ...
                for i in range(doc_length):
                    ...
                    topic_normalizers[old_topic] = 1.0 / (
                                self.topic_totals[old_topic] + self.smoothing_times_vocab_size)
                    ...
                    for topic in range(self.num_topics):
                        topic_probs[topic] = (doc_topic_counts[topic] + self.doc_smoothing) * (
                                    word_topic_counts[topic] + self.word_smoothing) * topic_normalizers[topic]
                    ...
                    topic_normalizers[new_topic] = 1.0 / (
                                self.topic_totals[new_topic] + self.smoothing_times_vocab_size)
    ...
    def write_keys(self, writer):
        for topic in range(self.num_topics):
            sorted_words = sorted(zip(self.word_topics[:, topic], self.vocabulary), reverse=True)
            writer.write(" ".join([w for x, w in sorted_words[:20]]))
            writer.write('\n')
            # print(" ".join([w for x, w in sorted_words[:20]]))


def sample_doc(long[:] doc_tokens, long[:]doc_topics, long[:]topic_changes, long[:]doc_topic_counts, \
               long[:, :]word_topics, long[:]topic_totals, double[:]topic_probs, double[:]topic_normalizers, \
               float doc_smoothing, float word_smoothing, float smoothing_times_vocab_size, int num_topics):
    ...
    for i in range(doc_length):
        ...
        for topic in range(num_topics):
            topic_probs[topic] = (doc_topic_counts[topic] + doc_smoothing) * (word_topic_counts[topic] + word_smoothing) * \
